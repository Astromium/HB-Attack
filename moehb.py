...
class MOEHB():
    ...
    def run(self):
        if self.history is None:
            ...
            classifier = Pipeline(steps=[('preprocessing', scaler), ('model', model)])
            constraints_executor = NumpyConstraintsExecutor(AndConstraint(self.constraints), feature_names=self.feature_names)
            ...
            for i, (score, config, candidate) in enumerate(tqdm(zip(scores, configs, candidates), total=len(scores))):
                ...
                fronts = fast_non_dominated_sort.fast_non_dominated_sort(np.array(score))
                ...
                res = minimize(problem, algorithm, termination=('n_gen', self.n_gen))
